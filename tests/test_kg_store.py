...
no API key is needed. Three groups:
  ...
  - LifeGraph-level: lg.log(user_id=TEST_USER) flow, lg.episodes.<...>, lg.query
...
def test_store_entity_dedup_by_type_and_key() -> None:
    ...
    sara = Person(user_id=TEST_USER, value="Sara", key="sara")
    s.save_episode(
        Episode(user_id=TEST_USER, id="ep-1", text="First", occurred_at=T0, ingested_at=T0),
        entities=[sara],
        edges=[],
    )
    s.save_episode(
        Episode(user_id=TEST_USER, id="ep-2", text="Second", occurred_at=T1, ingested_at=T1),
        entities=[sara],
        edges=[],
    )
...
def test_store_episodes_mentioning_uses_link_table() -> None:
    ...
    sara = Person(user_id=TEST_USER, value="Sara", key="sara")
    s.save_episode(
        Episode(user_id=TEST_USER, id="ep-1", text="Met Sara", occurred_at=T0, ingested_at=T0),
        entities=[sara],
        edges=[],
    )
    s.save_episode(
        Episode(user_id=TEST_USER, id="ep-2", text="Saw Tao", occurred_at=T1, ingested_at=T1),
        entities=[Person(user_id=TEST_USER, value="Tao", key="tao")],
        edges=[],
    )
...
def test_store_episodes_since_orders_by_time() -> None:
    s = SqliteStore(":memory:")
    s.save_episode(
        Episode(user_id=TEST_USER, id="old", text="old", occurred_at=T0, ingested_at=T0),
        entities=[],
        edges=[],
    )
    s.save_episode(
        Episode(user_id=TEST_USER, id="new", text="new", occurred_at=T2, ingested_at=T2),
        entities=[],
        edges=[],
    )
    ...
    # Filter cutoff
    eps_filtered = s.episodes_since(T1, user_id=TEST_USER)
...
def test_lifegraph_query_finds_persisted_entity() -> None:
    fake = FakeClient(extraction_response=_RAMEN_EXTRACTION)
    lg = LifeGraph(llm=fake)
    lg.log("Had ramen with Sara at Ippudo", occurred_at=T1, user_id=TEST_USER)
...
def test_lifegraph_query_topic_kind_filter() -> None:
    fake = FakeClient(extraction_response=_RAMEN_EXTRACTION)
    lg = LifeGraph(llm=fake)
    lg.log("Had ramen", occurred_at=T1, user_id=TEST_USER)
    foods = lg.query(Topic, kind="food", user_id=TEST_USER).all()
    assert len(foods) == 1
...
def test_episodes_between_returns_inclusive_range() -> None:
    s = SqliteStore(":memory:")
    s.save_episode(
        Episode(user_id=TEST_USER, id="early", text="early", occurred_at=T0, ingested_at=T0),
        entities=[],
        edges=[],
    )
    s.save_episode(
        Episode(user_id=TEST_USER, id="mid", text="mid", occurred_at=T1, ingested_at=T1),
        entities=[],
        edges=[],
    )
    s.save_episode(
        Episode(user_id=TEST_USER, id="late", text="late", occurred_at=T2, ingested_at=T2),
        entities=[],
        edges=[],
    )
    # Range that includes only the middle
    eps = s.episodes_between(T1, T1, user_id=TEST_USER)
    assert [e.id for e in eps] == ["mid"]
    # Range that spans all
    eps_all = s.episodes_between(T0, T2, user_id=TEST_USER)
    assert [e.id for e in eps_all] == ["late", "mid", "early"]


def test_entity_query_episodes_pivot() -> None:
    """`lg.query(Topic, kind='food', user_id=TEST_USER).episodes()` returns the meal timeline."""
    fake = FakeClient(extraction_response=_RAMEN_EXTRACTION)
    lg = LifeGraph(llm=fake)
    lg.log("Had ramen with Sara at Ippudo", occurred_at=T0, user_id=TEST_USER)
    lg.log("Had ramen with Sara at Ippudo", occurred_at=T1, user_id=TEST_USER)  # second meal
    food_episodes = lg.query(Topic, kind="food", user_id=TEST_USER).episodes()
...
def test_entity_query_episodes_pivot_dedups() -> None:
    ...
    sara_eps = lg.query(Person, key="sara", user_id=TEST_USER).episodes()
...
def test_lifegraph_in_memory_isolates_per_instance() -> None:
    ...
    a.log("Had ramen with Sara at Ippudo", occurred_at=T1, user_id=TEST_USER)
    assert len(a.query(Person, user_id=TEST_USER).all()) == 1
    assert len(b.query(Person, user_id=TEST_USER).all()) == 0
...
def test_invalidate_edge_supersede_not_delete() -> None:
    ...
    s = SqliteStore(":memory:")
    sara = Person(user_id=TEST_USER, value="Sara", key="sara")
    berlin = Place(user_id=TEST_USER, value="Berlin", key="berlin")
    tokyo = Place(user_id=TEST_USER, value="Tokyo", key="tokyo")

    # Initial fact: Sara lives in Berlin (logged June 2025)
    ep1 = Episode(
        user_id=TEST_USER, id="ep-berlin",
    ...
    facts_jan = s.edges_as_of(datetime(2025, 1, 1, tzinfo=UTC), verb="lives_in", user_id=TEST_USER)
    assert facts_jan == []
...
def test_edges_as_of_filters_by_verb() -> None:
    ...
    s._conn.commit()
    ate_edges = s.edges_as_of(T1, verb="ate", user_id=TEST_USER)
